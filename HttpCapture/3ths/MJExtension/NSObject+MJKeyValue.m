                ...
                [propertyKeys enumerateObjectsUsingBlock:^(MJPropertyKey *propertyKey, NSUInteger idx, BOOL *stop) {
                    ...
                    if (nextPropertyKey) { // 不是最后一个key
                        ...
                        if ([tempInnerContainer isKindOfClass:[NSMutableArray class]]) {
                            ...
                            while ([tempInnerContainer length] < index + 1) {
...
- (id)JSONObject
{
    if ([self isKindOfClass:[NSString class]])
    {
        NSData * data = [[((NSString *)self) dataUsingEncoding:NSUTF8StringEncoding] exchangeObject];
        return [NSJSONSerialization JSONObjectWithData:data options:kNilOptions error:nil];
    }
    else if ([self isKindOfClass:[NSData class]])
    {
        NSData * da = [(NSData *)self exchangeObject];
        return [NSJSONSerialization JSONObjectWithData:da options:kNilOptions error:nil];
    }
