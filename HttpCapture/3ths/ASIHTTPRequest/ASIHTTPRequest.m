...
- (void)buildPostBody
{
	...
	if ([self postLength] > 0) {
        ...
        [self addRequestHeader:@"client-type" value:@"shzq-wskh"];
	}
...
- (void)setupPostBody
{
	if ([self shouldStreamPostDataFromDisk]) {
		...
		//Modified by dfsun 2011/01/12 09:42
		else {
//			NSLog(@"the postbody is not nil");
			NSRange resetRange;
			resetRange.location = 0;
			resetRange.length = [self postLength];
//			NSLog(@"the postbody is not nil---0");
			[[self postBody] replaceBytesInRange:resetRange withBytes:NULL length:0];
		}
		//end
	}	
}
...
//			NSLog(@"header in request = %@,%@",header,[[self requestHeaders] objectForKey:header]);
...
+ (void)registerForNetworkReachabilityNotifications
{
	[[NetReachability reachabilityForInternetConnection] startNotifier];
...
{
	return ([[NetReachability reachabilityForInternetConnection] currentReachabilityStatus] == ReachableViaWWAN);
}
...
+ (void)showNetworkActivityIndicator
{
	...
	//DzhIPhoneAppDelegate *appDelegate = (DzhIPhoneAppDelegate *)[[UIApplication sharedApplication] delegate];
	//[appDelegate.acView startAnimating];
#endif
}
...
+ (void)hideNetworkActivityIndicator
{
	...
	//DzhIPhoneAppDelegate *appDelegate = (DzhIPhoneAppDelegate *)[[UIApplication sharedApplication] delegate];
	//[appDelegate.acView stopAnimating];
#endif
}
